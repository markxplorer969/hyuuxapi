...
import { Book, TrendingUp, Bot, Download, Image, Sparkles, Zap, Code, Menu, X, ExternalLink, Sun, Moon } from 'lucide-react'
...
const Home = ({ metadata }) => {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950 text-gray-100' : 'bg-white text-gray-900'} transition-colors duration-300`}>
      {/* Navbar */}
      <nav className={`fixed top-0 left-0 right-0 z-50 ${isDarkMode ? 'bg-slate-900/95 border-slate-800' : 'bg-white/95 border-gray-200'} backdrop-blur-sm border-b`}>
        <div className="container mx-auto px-4 md:px-8 max-w-7xl">
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <button
              onClick={toggleTheme}
              className={`p-2 rounded-lg ${isDarkMode ? 'bg-slate-800 hover:bg-slate-700' : 'bg-gray-100 hover:bg-gray-200'} transition-colors`}
      ...
      <div className="pt-16 md:pt-20">
        ...
        <section className="relative min-h-[calc(100vh-4rem)] flex items-center justify-center py-12 md:py-20 px-4">
          ...
          <div className="absolute inset-0 overflow-hidden">
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] md:w-[500px] h-[300px] md:h-[500px] ${isDarkMode ? 'bg-blue-500/10' : 'bg-blue-200/30'} rounded-full blur-3xl animate-pulse`}></div>
          </div>

          <div className="container mx-auto max-w-7xl relative z-10">
            <div className="text-center max-w-4xl mx-auto">
              {/* Badge */}
              <div className={`inline-flex items-center gap-2 px-4 md:px-6 py-2 md:py-3 ${isDarkMode ? 'bg-gradient-to-r from-blue-500/10 to-purple-500/10 border-blue-500/30' : 'bg-gradient-to-r from-blue-100 to-purple-100 border-blue-300'} border rounded-full mb-6 md:mb-8`}>
              ...
              {/* Subtitle */}
              <p className={`text-base md:text-xl ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mb-8 md:mb-10 max-w-3xl mx-auto`}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center">
                ...
                <button
                  ...
                  className={`inline-flex items-center justify-center gap-2 px-6 md:px-8 py-3 md:py-4 ${isDarkMode ? 'bg-slate-800 hover:bg-slate-700' : 'bg-gray-100 hover:bg-gray-200'} rounded-lg font-medium transition-colors text-sm md:text-base`}
                >
        ...
        {/* Features Section */}
        <section className={`py-12 md:py-20 ${isDarkMode ? 'bg-slate-900/50' : 'bg-gray-50'} px-4`}>
          <div className="container mx-auto max-w-7xl">
            <div className="text-center mb-12 md:mb-16">
              ...
              <p className={`text-base md:text-xl ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} max-w-2xl mx-auto`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {features.map((feature, index) => (
                <div key={index} className={`${isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-blue-500/50' : 'bg-white border-gray-200 hover:border-blue-300'} border rounded-xl p-6 transition-colors group`}>
                  ...
                  <p className={`text-sm md:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} leading-relaxed`}>{feature.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-12 md:py-20 px-4">
          <div className="container mx-auto max-w-7xl">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
              <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-blue-500/50' : 'bg-white border-gray-200 hover:border-blue-300'} border rounded-xl p-4 md:p-6 text-center transition-colors`}>
                ...
                <div className={`text-xs md:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>API Endpoints</div>
              </div>
              <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-blue-500/50' : 'bg-white border-gray-200 hover:border-blue-300'} border rounded-xl p-4 md:p-6 text-center transition-colors`}>
                <div className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-blue-400 to-purple-600 bg-clip-text text-transparent mb-2">
                  {stats.totalRequests}
                </div>
                <div className={`text-xs md:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Total Requests</div>
              </div>
              <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-blue-500/50' : 'bg-white border-gray-200 hover:border-blue-300'} border rounded-xl p-4 md:p-6 text-center transition-colors`}>
                <div className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-blue-400 to-purple-600 bg-clip-text text-transparent mb-2">
                  {stats.uptime}
                </div>
                <div className={`text-xs md:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Uptime</div>
              </div>
              <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800 hover:border-blue-500/50' : 'bg-white border-gray-200 hover:border-blue-300'} border rounded-xl p-4 md:p-6 text-center transition-colors`}>
                <div className="text-3xl md:text-5xl font-bold bg-gradient-to-r from-blue-400 to-purple-600 bg-clip-text text-transparent mb-2">
                  {stats.responseTime}
                </div>
                <div className={`text-xs md:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Avg Response</div>
              </div>
            </div>
          </div>
        </section>
      ...
      {/* Footer */}
      <footer className={`${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-gray-100 border-gray-200'} border-t py-8 md:py-12 px-4`}>
        <div className="container mx-auto max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12">
            ...
            <div>
              ...
              <ul className="space-y-2">
                <li>
                  <button
                    onClick={() => navigateTo('/docs')}
                    className={`text-sm md:text-base ${isDarkMode ? 'text-gray-400 hover:text-blue-400' : 'text-gray-600 hover:text-blue-600'} transition-colors flex items-center gap-2`}
                  >
            ...
            <div>
              ...
              <ul className="space-y-2">
                ...
                <li>
                  <span className={`text-sm md:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>API Version: 1.0</span>
                </li>
                <li>
                  <span className={`text-sm md:text-base ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Uptime: {stats.uptime}</span>
                </li>
              </ul>
            </div>
          </div>
          ...
          {/* Bottom Bar */}
          <div className={`mt-8 md:mt-12 pt-6 md:pt-8 ${isDarkMode ? 'border-slate-800' : 'border-gray-200'} border-t text-center`}>
