...
export class Tripay {
  ...
  getExpiredTimestamp(hours = 24) {
        const now = new Date();
        const expiredDate = new Date(now.getTime() + (hours * 60 * 60 * 1000));
        return Math.floor(expiredDate.getTime() / 1000);
    }
  ...
  async createPayment(params: {
    ...
    try {
      ...
      const expiredTimestamp = this.getExpiredTimestamp(1);
